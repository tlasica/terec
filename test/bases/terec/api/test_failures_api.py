...
class TestFailuresGetFailedTestsAPI:
    ...
    def get_failed_tests(self, org, project, suite, branch, headers=None):
        ...
        return self.api_client.get(url, params=params, headers=headers)
    ...
    def test_should_return_failed_tests(self, cassandra_model, public_project):
    ...
    def test_authz(self, cassandra_model, private_project):
        ...
        org = prj.org
        resp = self.get_failed_tests(org, prj.name, "s", "main")
...
class TestFailuresGetTestRunsAPI:
    ...
    def get_test_runs(
        ...
        return self.api_client.get(url, params=q_params, headers=headers)
    ...
    def test_should_return_single_test_history(self, cassandra_model, public_project):
        # given some generated data with failed tests
        branch = "main"
        suite, suite_runs, test_runs = generate_suite_with_test_runs(
            public_project.org, public_project.name, branch=branch
        )
    ...
    def test_should_return_class_history(self, cassandra_model, public_project):
        # given some generated data with failed tests
        suite, suite_runs, test_runs = generate_suite_with_test_runs(
            public_project.org, public_project.name, branch="main"
        )
        assert len(test_runs) > 0
        # when we get single test history
        the_test: TestCaseRun = test_runs[0]
    ...
    def test_should_return_package_history(self, cassandra_model, public_project):
        # given some generated data with failed tests
        suite, suite_runs, test_runs = generate_suite_with_test_runs(
            public_project.org, public_project.name, branch="main"
        )
    ...
    def test_authz(self, cassandra_model, private_project):
        prj, tokens = private_project
        resp = self.get_test_runs(prj.org, prj.name, "s", "main", "p")
        assert resp.status_code == 401
...
class TestFailuresCheckTestRunAPI:
    ...
    def test_get_test_run_check(self, cassandra_model, public_project):
        suite, suite_runs, test_runs = generate_suite_with_test_runs(
            public_project.org, public_project.name, branch="main"
        )
    ...
    def test_check_known_fail_on_same_branch(self, cassandra_model, public_project):
        pass

    def test_check_new_fail_on_same_branch(self, cassandra_model, public_project):
        pass
